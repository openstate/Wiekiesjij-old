				...
				<?php
					...
					if(count($_POST['party']) == 0) {
						...
						while($r = $q->fetch()) {
							...
							if($_POST['burgerlijkestaat']) {
								$accum += $_POST['weight6'];
								$gezin = $_POST['weight6'] . "@{";
								foreach ($_POST['burgerlijkestaat'] as $bid) {
									$gezin .= $bid .",";
									if($bid == $r['burgerlijkestaat']) {
										$score[$r['id']] += $_POST['weight6'];
									}
								}
								if (substr($gezin, -1) == ",") {
									$gezin = substr_replace($gezin,"",-1);
								}
								$gezin .= "}";
							}
							...
							if($_POST['geloof']) {
								$accum += $_POST['weight7'];
								$overt = $_POST['weight7'] . "@{";
								foreach ($_POST['geloof'] as $gid) {
									$overt .= $gid .",";
									if($gid == $r['geloof']) {
										$score[$r['id']] += $_POST['weight7'];
									}
								}
								if (substr($overt, -1) == ",") {
									$overt = substr_replace($overt,"",-1);
								}
								$overt .= "}";
							}
							...
							if($_POST['burgerlijkestaat']) {
								$accum += $_POST['weight7'];
								$gel = $_POST['weight7'] . "@{";
								foreach ($_POST['gemeenschap'] as $gid) {
									$gel .= $gid .",";
									if($gid == $r['gemeenschap']) {
										$score[$r['id']] += $_POST['weight7'];
									}
								}
								if (substr($gel, -1) == ",") {
									$gel = substr_replace($gel,"",-1);
								}
								$gel .= "}";
							}
 ...
 ************************************************************/
					...
					$bzg = (is_numeric($_POST['bez_geslacht']))?$_POST['bez_geslacht']:-1;
					$bzl = (is_numeric($_POST['bez_leeftijd']))?$_POST['bez_leeftijd']:-1;
					$bzs = (is_numeric($_POST['bez_vrkstem']))?$_POST['bez_vrkstem']:-1;
					$bza = base64_encode($_POST['bez_andere']);
									
					
					$record .= "}"; 
					// om te beveiligen worden email en naam in BASE64 opgeslagen
					$sql2 = "INSERT INTO `ns_kiezerdata` (`naam` , `email` , `region`, `partijen`, `geslacht`, `leeftijd`, `expertise`, `sector`, `gezin`, `overtuig`, `geloof`, `opleiding`, `ervaring`, `eerstekamer`, `bez_geslacht`, `bez_leeftijd`, `bez_vrkstem`, `bez_andere`)
 						VALUES ('".base64_encode($_POST['eerstekamernaam'])."','".base64_encode($_POST['eerstekamermail'])."', $region, '$record', '".$gesl."', '$leeftd', '$exprt', '$sect', '$gezin', '$overt', '$gel', '$opl','$erva', '$eerstekamer', $bzg, $bzl, $bzs, $bza)";
//					echo "<!-- $sql2 -->\n";
		...
		<?php echo $fotos; ?>
            ...
            <div style="text-align: center"><a href="http://www.primaries.nl/vrkst_vote.php?c=<?php echo substr($votelink,0,-1); ?>"><img src="images/stemopdeze.gif"></a></div> 
