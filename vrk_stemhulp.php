...
foreach ($sw_score as $id=>$sc) {
	echo "<input type=\"hidden\" name=\"kandidaat_".$id."\" value=\"".floor($sw_score[$id])."\">\n";
	if ($count < 10) {
//		break;
//	} else {
		$count++;
...
?>
					<div id="page<?php $i++; echo $i;?>" class="votehelppage" style="display: none;">
					        ...
					        <div class="box">
					      	  <div class="dummy"><h1> Inhoud <> persoon </h1></div>
						  <div class="voortgang">[10 / 10]</div>
						</div>
<?php
 $q = new DBQuery("SELECT content FROM ns_static WHERE id=26");
 $r = $q->fetch();
 echo base64_decode($r['content']);
?>	
...
?>
					...
					<div id="page<?php $i++; echo $i;?>" class="votehelppage" style="display: none;">
					        ...
					        <div class="box">
					      	  <div class="dummy"><h1> Enqu&ecirc;te </h1></div>
						  <div class="voortgang">extra</div>
						</div>
<?php
 $q = new DBQuery("SELECT content FROM ns_static WHERE id=25");
 $r = $q->fetch();
 echo base64_decode($r['content']);
?>
						<img src="images/stippellijn.gif" alt=""/>
